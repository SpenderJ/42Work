...
static int		ft_less(t_asm *s_asm, int line, int actual, int val)
{
	...
	while (line < actual)
	{
		...
		while (s_asm->file[line][i] == ' ' || s_asm->file[line][i] == '\t')
			i++;
		ft_op_dir(s_asm, s_asm->file[line] + i, &opc, &dir);
			...
			COMMENT_CHAR)
		{
			...
			val += ft_count(s_asm, s_asm->file[line] + i, dir, 0);
		}
		line++;
	}
	return (val);
...
static int		ft_greater(t_asm *s_asm, int line, int actual, int val)
{
	...
	while (line > actual)
	{
		...
		while (s_asm->file[actual][i] == ' ' || s_asm->file[actual][i] == '\t')
			i++;
		ft_op_dir(s_asm, s_asm->file[actual] + i, &opc, &dir);
		...
		{
			val = (opc == 1 ? val + 2 : val + 1);
			val += ft_count(s_asm, s_asm->file[actual] + i, dir, 0);
		}
		actual++;
	}
	return (val);
...
unsigned int	ft_find_label(t_asm *s_asm, char *s, int first, int actual)
{
