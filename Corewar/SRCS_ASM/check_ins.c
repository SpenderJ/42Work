...
static char	ft_dir(char *s, char *nbr, char *inst, int *index)
{
	int i;
...
static char	ft_direct(char *s, char *nbr, char *inst, int *index)
{
	...
	{
		if (ft_check_dir_else(s, nbr, inst, &i) == 0)
		{
			*index = i;
			return (0);
		}
...
void		ft_check_instruction_line(char *s, char nbr, char *inst)
{
	...
	i = 0;
	(inst[nbr] != 0) ? ft_exit("Wrong instruction") : 0;
	tmp = 0;
