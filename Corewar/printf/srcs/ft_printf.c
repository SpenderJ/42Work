...
/*   Updated: 2018/01/27 18:51:46 by juspende         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		precision_pars(va_list argp, const char *arg, int *index, t_flag *flag)
{
	...
	if (arg[0] == '.' && (flag->comma = 1) == 1)
	{
		...
		else if ((flag->point = ft_atoii(&arg[c])) == -1)
			return (0);
	...
	else
	{
		if (arg[(c = 0)] == '*')
			(flag->larg = va_arg(argp, int)) < 0 ?
				(flag->neg = 1) : flag->neg;
		arg[c] != '*' ? (flag->larg = ft_atoii(&arg[c])) : flag->larg;
