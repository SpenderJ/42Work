...
{
	write(1, flag->printed, fttt_strlen(flag->printed));
	free(flag->printed);
	flag->printed = ft_strneeew(2);
	return ;
}
...
int			s(va_list argp, const char *arg, int *index, t_flag *flag)
{
	...
	!tmp && !flag->comma ? (tmp = ft_strduuup("(null)")) :
		(tmp = ft_strduuup(tmp));
	if (flag->comma && (int) fttt_strlen(tmp) > flag->point && flag->point > 0)
		tmp[flag->point] = '\0';
	if (flag->comma && flag->point <= 0 && (b = -1))
		;
	b == -1 ? (flag->larg += fttt_strlen(tmp)) : flag->larg;
	flag->larg = flag->larg - fttt_strlen(tmp);
